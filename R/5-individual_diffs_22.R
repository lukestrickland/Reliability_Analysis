...
#Remove participants 12 and 21 due to convergence issue

full_balance <- full_balance[!full_balance %in% c("12", "21")]
...
save(ex_L_benefit, inh_L_benefit, ex_H_benefit, inh_H_benefit,
  ...
  file= "model_correlations_22.RData")
...
CA_top_samples <- CA_top_samples[!(names(CA_top_samples) %in% c("4"))]
...
all_effects$Condition <- "Low Reliability"
all_effects$Condition[grep("H", all_effects$effect)] <- "HIgh Reliability"
