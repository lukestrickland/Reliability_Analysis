...
# 
# pp <- h.post.predict.dmc(CA_top_samples, save.simulation = TRUE, cores=length(CA_top_samples))
# 
# theme_set(theme_simple())
# 
...
# 
# save(msds, file=
#       "samples_data/msds_top_samples_balance.RData")
# 
# msds <-
...
Table 2 contains our statistical tests of excitation and inhibition effects. We found particularly strong effects in the high-reliability condition for both excitation and inhibition. In the low-reliability condition, we also found weaker (although still substantial) effects, with inhibition effects being
greater than excitation effects.
...
inhextab <- rbind(
  ...
  c(L_conf_ex_success, L_nonconf_inh_success, H_conf_ex_success, H_nonconf_inh_success),
  c(L_conf_ex_fail, L_nonconf_inh_fail, H_conf_ex_fail, H_nonconf_inh_fail)
...
Threshold estimates are plotted in Figure 9. Statistical tests of differences across automated and manual conditions in thresholds are tabulated in Table 3. Low-reliability automation had relatively little effect on thresholds, consistent with Strickland et al. (2021). However, there were some small drops in threshold levels compared with the manual condition, suggesting participants may have become less cautious when provided with automation. Threshold decreases were much stronger in the high-reliability condition, suggesting a substantial decrease in caution when participants were provided high-reliability
automation.
