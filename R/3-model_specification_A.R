...
CA_top_p.prior <- prior.p.dmc(
  dists = rep("tnorm", length(CA_top_p.vector)),
  p1=CA_top_p.vector,                           
  p2=c(1,0.25,1,rep(1, 18), rep(2, 24)),
  lower=c(0.1, 0.01,0, rep(0, 18), rep(NA, 24)),
  upper=c(5,10, rep(Inf, length(CA_top_p.vector)-2))
)
...
CA_top_samples_A_lb <- h.samples.dmc(nmc = 180,
                                  CA_top_p.prior,
                                  CA_top_dm, thin=20)

save(CA_top_samples_A_lb, file="CA_top_samples_A_lb.RData")
