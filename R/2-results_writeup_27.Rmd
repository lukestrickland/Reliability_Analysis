...
plot.df$Condition <- factor(plot.df$Condition, 
                             levels=c("Manual", "Automation_Low", "Automation_High"),
                             labels= c("Manual", "Low Reliability", "High Reliability"))
...
H_mRTs_success_effect$effect <- "Benefit"
H_mRTs_failure_effect$effect <- "Cost"

H_mRTs_success_effect$cond <- "High Reliability"
H_mRTs_failure_effect$cond <- "High Reliability"


L_mRTs_success_effect$effect <- "Benefit"
L_mRTs_failure_effect$effect <- "Cost"

L_mRTs_success_effect$cond <- "Low Reliability"
L_mRTs_failure_effect$cond <- "Low Reliability"




all_effects <- rbind(H_mRTs_success_effect,
                     H_mRTs_failure_effect,
                     L_mRTs_success_effect,
                     L_mRTs_failure_effect)

all_effects$s <- factor(as.numeric(all_effects$s))


ggplot(all_effects, aes(s, mRTboost)) +
  geom_point(stat = "identity", size=2.5) +
  geom_hline(aes(yintercept=0), linetype=2)+
  facet_grid(cond~effect) +xlab("Participant") +ylab ("Effect") +
...
H_accs_success_effect$effect <- "Benefit"
H_accs_failure_effect$effect <- "Cost"

H_accs_success_effect$cond <- "High Reliability"
H_accs_failure_effect$cond <- "High Reliability"


L_accs_success_effect$effect <- "Benefit"
L_accs_failure_effect$effect <- "Cost"

L_accs_success_effect$cond <- "Low Reliability"
L_accs_failure_effect$cond <- "Low Reliability"




all_effects <- rbind(H_accs_success_effect,
                     H_accs_failure_effect,
                     L_accs_success_effect,
                     L_accs_failure_effect)

all_effects$s <- factor(as.numeric(all_effects$s))


ggplot(all_effects, aes(s, accboost)) +
  geom_point(stat = "identity", size=2.5) +
  geom_hline(aes(yintercept=0), linetype=2)+
  facet_grid(cond~effect) +xlab("Participant") +ylab ("Effect") +
  theme(text = element_text(size = 14), axis.text.x=element_blank())
