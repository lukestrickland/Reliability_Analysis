...
```{r check_failtrial_H, echo= FALSE , results = "hide", message=FALSE, warning=FALSE}

checkman <- cleandats %>% filter(Condition=="Manual")

checkman$fails <- ifelse(checkman$Automation=="Automation Success",
                         "Succ",
                         "LFail")
checkman$fails[checkman$failtrial_H] <- "HFail"

checkman_glmer <-
  glmer(C ~ fails + (1 |s),
        data = checkman %>% filter(fails!="Succ"),
        family = binomial(link = "probit"))

Anova(checkman_glmer,type="II")

checkman_glmer <-
  glmer(C ~ fails*Stimulus*Session + (1 |s),
        data = checkman %>% filter(fails!="Succ"),
        family = binomial(link = "probit"))

Anova(checkman_glmer,type="II")

checkman_RTs <- checkman %>% filter(fails!="Succ") %>% 
  group_by(s, fails) %>% 
  filter(C) %>% 
  summarise(RT=mean(RT))

checkman_RT_lmer <- lmer(RT ~ fails + (1 |s),
     data = checkman_RTs )

Anova(checkman_RT_lmer, type="II")


checkman_RTs <- checkman %>% filter(fails!="Succ") %>% 
  group_by(s, fails, Stimulus, Session) %>% 
  filter(C) %>% 
  summarise(RT=mean(RT))

checkman_RT_lmer <- lmer(RT ~ fails*Stimulus*Session + (1 |s),
     data = checkman_RTs )
...
acc_glmer_random <-
  glmer(C ~ Stimulus * Condition * Session * Automation + (1 +Stimulus * Condition * Session * Automation|s),
        data = cleandats,
        family = binomial(link = "probit"))

save(acc_glmer_random, file = "img/acc_model_randomslopes.RData")
...
```{r RT_session, echo= FALSE, message=FALSE, warning=FALSE, results="asis",fig.height = 3, fig.width = 4}
...
```{r trust, echo= FALSE, message=FALSE, warning=FALSE, results="asis"}
...
trust_lmer <- lmer(score ~ Condition * Session + (1 |Participant),
     data = trust_scores_p )

pandoc.table(
  make_model_table(Anova(trust_lmer , type="II")))
